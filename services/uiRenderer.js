    ...
    renderRecommendations(recommendations) {
        ...
        const compatibleUniversities = Array.isArray(recommendations.universities)
            ? recommendations.universities
                .map(uni => ({ raw: uni, data: this.normalizeItem(uni) }))
                .filter(entry => entry.data && entry.data.matched.length > 0)
            : [];

        if (compatibleUniversities.length > 0) {
            html += '<h3 style="color: var(--accent); margin-top: 20px; margin-bottom: 15px;">🏫 Où étudier ça</h3>';

            compatibleUniversities.forEach(({ raw: uni, data: uniData }) => {
        ...
        } else if (Array.isArray(recommendations.universities)) {
            html += '<h3 style="color: var(--accent); margin-top: 20px; margin-bottom: 15px;">???? O?? ??tudier ??a</h3>';
            html += '<div class="rec-card clean" style="text-align: center; opacity: 0.7;">';
            html += '<p style="margin: 0; color: #94a3b8;">Aucune universit?? ne propose ces fili??res pour le moment.</p>';
            html += '</div>';
        }

        const compatibleCentres = Array.isArray(recommendations.centres)
            ? recommendations.centres
                .map(centre => ({ raw: centre, data: this.normalizeItem(centre) }))
                .filter(entry => entry.data && entry.data.matched.length > 0)
            : [];

        if (Array.isArray(recommendations.centres)) {
            if (compatibleCentres.length > 0) {
                html += '<h3 style="color: var(--accent); margin-top: 20px; margin-bottom: 15px;">???? Formations rapides</h3>';

                compatibleCentres.forEach(({ raw: centre, data: centreData }) => {
